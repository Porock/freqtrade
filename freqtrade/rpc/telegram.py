...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "ru": {
        ...
        "Statistics": "Статистика",
        # Help command lines
        "/start: Starts the trader": "Запускает трейдера",
        "/pause: Pause the new entries for trader, but handles open trades gracefully": "Приостанавливает новые входы, открытые позиции обрабатываются штатно",
        "/stop: Stops the trader": "Останавливает трейдера",
        "/stopentry: Stops entering, but handles open trades gracefully": "Запрещает новые входы, открытые позиции обрабатываются штатно",
        "/forceexit <trade_id>|all: Instantly exits the given trade or all trades, regardless of profit": "Немедленно закрывает указанную/все сделки, независимо от профита",
        "/fx <trade_id>|all: Alias to /forceexit": "Алиас к /forceexit",
        "/forcelong <pair> [<rate>]: Instantly buys the given pair. Optionally takes a rate at which to buy (only applies to limit orders).": "Немедленно покупает указанную пару. Можно указать цену (для лимитных ордеров).",
        "/forceshort <pair> [<rate>]: Instantly shorts the given pair. Optionally takes a rate at which to sell (only applies to limit orders).": "Немедленно шортит указанную пару. Можно указать цену (для лимитных ордеров).",
        "/delete <trade_id>: Instantly delete the given trade in the database": "Немедленно удаляет указанную сделку из БД",
        "/reload_trade <trade_id>: Reload trade from exchange Orders": "Перезагружает сделку из ордеров биржи",
        "/cancel_open_order <trade_id>: Cancels open orders for trade. Only valid when the trade has open orders.": "Отменяет открытые ордера для сделки. Доступно только при наличии открытых ордеров.",
        "/coo <trade_id>|all: Alias to /cancel_open_order": "Алиас к /cancel_open_order",
        "/whitelist [sorted] [baseonly]: Show current whitelist. Optionally in order and/or only displaying the base currency of each pairing.": "Показ текущего вайтлиста. Опционально — сортировка и/или только базовые валюты.",
        "/blacklist [pair]: Show current blacklist, or adds one or more pairs to the blacklist.": "Показ текущего блэклиста или добавление пары(пар) в блэклист.",
        "/blacklist_delete [pairs]| /bl_delete [pairs]: Delete pair / pattern from blacklist. Will reset on reload_conf.": "Удаляет пару/шаблон из блэклиста. Сбросится при reload_conf.",
        "/reload_config: Reload configuration file": "Перезагрузка конфигурации",
        "/unlock <pair|id>: Unlock this Pair (or this lock id if it's numeric)": "Разблокировать пару (или блокировку по id)",
        "/show_config: Show running configuration": "Показ текущей конфигурации",
        "/locks: Show currently locked pairs": "Показ текущих блокировок",
        "/balance: Show bot managed balance per currency": "Балансы под управлением бота по валютам",
        "/balance total: Show account balance per currency": "Баланс аккаунта по валютам",
        "/logs [limit]: Show latest logs - defaults to 10": "Показ последних логов (по умолчанию 10)",
        "/count: Show number of active trades compared to allowed number of trades": "Показ числа активных сделок и лимита",
        "/health Show latest process timestamp - defaults to 1970-01-01 00:00:00": "Показ метки времени последнего процесса",
        "/marketdir [long | short | even | none]: Updates the user managed variable that represents the current market direction. If no direction is provided the currently set market direction will be output.": "Обновляет пользовательское направление рынка. Без аргумента — вывод текущего значения.",
        "/list_custom_data <trade_id> <key>: List custom_data for Trade ID & Key combo. If no Key is supplied it will list all key-value pairs found for that Trade ID.": "Показ custom_data по сделке и ключу. Без ключа — все пары ключ/значение по сделке.",
        "/status <trade_id>|[table]: Lists all open trades": "Список всех открытых сделок",
        "<trade_id> : Lists one or more specific trades.                        Separate multiple <trade_id> with a blank space.": "Показ одной или нескольких сделок. Несколько id разделяются пробелом.",
        "table : will display trades in a table                pending buy orders are marked with an asterisk (*)                pending sell orders are marked with a double asterisk (**)": "показ в виде таблицы. Ожидающие покупки помечены *, продажи — **",
        "/entries <pair|none>: Shows the enter_tag performance": "Эффективность меток входа",
        "/exits <pair|none>: Shows the exit reason performance": "Эффективность причин выхода",
        "/mix_tags <pair|none>: Shows combined entry tag + exit reason performance": "Комбинированная эффективность меток входа и причин выхода",
        "/trades [limit]: Lists last closed trades (limited to 10 by default)": "Последние закрытые сделки (по умолчанию 10)",
        "/profit [<n>]: Lists cumulative profit from all finished trades, over the last n days": "Совокупная прибыль по всем завершенным сделкам за последние n дней",
        "/profit_long [<n>]: Lists cumulative profit from all finished long trades, over the last n days": "Совокупная прибыль по лонгам за последние n дней",
        "/profit_short [<n>]: Lists cumulative profit from all finished short trades, over the last n days": "Совокупная прибыль по шортам за последние n дней",
        "/performance: Show performance of each finished trade grouped by pair": "Результаты завершённых сделок по парам",
        "/daily <n>: Shows profit or loss per day, over the last n days": "Прибыль/убыток по дням за последние n дней",
        "/weekly <n>: Shows statistics per week, over the last n weeks": "Статистика по неделям за последние n недель",
        "/monthly <n>: Shows statistics per month, over the last n months": "Статистика по месяцам за последние n месяцев",
        "/stats: Shows Wins / losses by Sell reason as well as Avg. holding durations for buys and sells.": "Победы/проигрыши по причинам выхода и средняя длительность удержания",
